            ...
            ref.orderByChild("userid").equalTo(userid.trim()).addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if(snapshot.exists()){
                        ...
                        if (password.equals(dbPassword) && role.equals("Parent")) {
                            Toast.makeText(ParentLogin.this, "Login Successfully", Toast.LENGTH_SHORT).show();
                            MySharedPreference.getInstance(ParentLogin.this).setUserLoginType(AppConstant.PARENT_TYPE);
                            Intent fp = new Intent(getApplicationContext(), ParentDashboard.class);
                            startActivity(fp);
                            finish();
                        } else {
